...
public class Captcha {
	...
	private void setValue(String value) {
		if (value.length() <=0 || value.length() >= MAX_LONG)
			throw new IllegalArgumentException(
					"Catpcha.setValue: longuitud de cadena no comprendida entre 0.."+MAX_LONG);
		this.value = value;
	}
	...
	private void setWidth(Integer w) {
		if (w < 0 || w > MAX_WIDTH)
			throw new IllegalArgumentException(
					"Catpcha.setWidth: valor no comprendido entre 0.." + MAX_WIDTH);
		this.width = w;
	}
	...
	private void setHeight(Integer h) {
		if (h < 0 || h > MAX_HEIGHT)
			throw new IllegalArgumentException(
					"Catpcha.setHeight: valor no comprendido entre 0.."+MAX_HEIGHT);
		this.height = h;
	}
	...
	public Captcha(String value, String fileName) {
		setValue(value);
	...
	public Captcha(String value, String fileName, Integer width, Integer height) {
		setValue(value);
		setFilename(fileName);
		setWidth(width);
		setHeight(height);
	...
	public void createJPG(){
		...
		for (int i = 0; i < 15; i++)
			// Generamos algo de ruido aditivo
		...
		for (int i = 0; i < 20; i++)
			// Generamos algo de ruido substractivo
			g.drawLine(trnd.nextInt(getWidth()), trnd.nextInt(getHeight()),
					trnd.nextInt(getWidth()), trnd.nextInt(getHeight()));
